...
public class TelaEnfase extends javax.swing.JFrame {
    ...
    public TelaEnfase(Map<Integer, Enfase> arvoreEnfase) {
        initComponents();
    ...
    private void mostrarNaGrid(){
        try {
            Iterator<Enfase> iterador = arvoreEnfase.values().iterator();
            DefaultTableModel model =  (DefaultTableModel) jTable_Saida.getModel();
            model.setNumRows(0);
            while(iterador.hasNext()){
                Enfase objetoAuxiliar = iterador.next();
                String[] linha = new String[2];
                linha[0] = objetoAuxiliar.getIdEnfase()+"";
    ...
    private void jButton_IncluirActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton_IncluirActionPerformed
        ...
        try {
            ...
            if(arvoreEnfase.containsKey(Integer.valueOf(jTextField_ID.getText()))) throw new Exception("Já existe uma Enfase com esse ID");
            ...
            Enfase novaEnfase = new Enfase(Integer.parseInt(jTextField_ID.getText()), jTextField_Descricao.getText().toUpperCase());
            arvoreEnfase.put(novaEnfase.getIdEnfase(), novaEnfase);
            limparDados();
            mostrarNaGrid();
        } catch (Exception erro) {
    ...
    private void jButton_AlterarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton_AlterarActionPerformed
        ...
        try {
            if(jTextField_ID.getText().isEmpty()) throw new Exception("ID vazio");
            if(!arvoreEnfase.containsKey(Integer.valueOf(jTextField_ID.getText()))) throw new Exception("Não existe uma Enfase com esse ID");
            if(!jTextField_ID.getText().matches("\\d+")) throw new Exception("O ID deve ser um número");
            if(jTextField_Descricao.getText().isEmpty()) throw new Exception("Descrição Vazia");
